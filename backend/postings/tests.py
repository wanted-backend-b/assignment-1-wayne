...
class FreeBoardListView(TestCase):
    ...
    def test_success_list_view_get(self):
        ...
        response = client.get(
            "/postings/freeboards", **header, content_type="application/json"
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json(),
            {"results": [{"id": 1, "title": "Hi", "context": "Hi", "views": 1}]},
        )
...
class FreeBoardDetailView(TestCase):
    ...
    def test_success_detail_view_get(self):
        ...
        header = {"HTTP_Authorization": self.token}
        response = client.get(
            "/postings/freeboards/detail/1", **header, content_type="application/json"
        )
    ...
    def test_success_detail_view_post(self):
        client = Client()
        header = {"HTTP_Authorization": self.token}
        body = {"title": "Hello", "context": "Hello There"}

        response = client.post(
            "/postings/freeboards/detail",
            **header,
...
class FreeBoardCommentView(TestCase):
    ...
    def test_success_comment_view_post(self):
        client = Client()
        header = {"HTTP_Authorization": self.token}
        body = {"comment": "Hello", "posting_id": 1}

        response = client.post(
            "/postings/freeboards/comment",
            **header,
            content_type="application/json",
            data=json.dumps(body)
        )
