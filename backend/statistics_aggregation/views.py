...
class OperateAgeView(View):
    def get(self, request):
        ...
        for i in range(1, 10):
            age_num = OperatingView.objects.select_related('user')\
                        .filter(user__age__gte=i*10, user__age__lt=i*10+10)\
                        .distinct().values_list('user').count()
            age_key = str(i*10)
...
class NoticeAgeView(View):
    def get(self, request):
        ...
        for i in range(1, 10):
            age_num = NoticeView.objects.select_related('user')\
                        .filter(user__age__gte=i*10, user__age__lt=i*10+10)\
                        .distinct().values_list('user').count()
            age_key = str(i*10)
...
class FreeAgeView(View):
    def get(self, request):
        ...
        for i in range(1, 10):
            age_num = FreeView.objects.select_related('user')\
                        .filter(user__age__gte=i*10, user__age__lt=i*10+10)\
                        .distinct().values_list('user').count()
            age_key = str(i*10)
